...
<form class="form-comment<?php echo $reply ? ' on-reply' : ""; ?>" id="<?php echo $c['anchor'][1]; ?>" action="<?php echo $url->clean . '/' . $c['path'] . $url->query('&amp;'); ?>" method="post">
  ...
  <?php else: ?>
    ...
    <p class="form-comment-input form-comment-input:email">
      ...
      <span>
        <input class="input width" id="form-comment-input:email" name="email" placeholder="<?php echo $language->commentPlaceholderMail; ?>" type="email" required>
      </span>
    </p>
  ...
  <div class="form-comment-textarea form-comment-textarea:content p">
    ...
    <div>
      <textarea class="textarea width" id="form-comment-textarea:content" name="content" placeholder="<?php echo To::text($reply ? $language->commentPlaceholderReply([$reply->author . ""], true) : $language->commentPlaceholderContent); ?>" required></textarea>
    </div>
  </div>
