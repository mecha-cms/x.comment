...
<form class="form-comment<?= $reply ? ' is:reply' : ""; ?>" id="<?= $c['anchor'][1]; ?>" action="<?= $url . '/.comment' . ($url->path ?? State::get('path')) . $url->query('&amp;'); ?>" method="post">
  ...
  <p class="form-comment-button form-comment-button:x p">
    ...
    <span>
      <button class="button type:submit" id="form-comment-button:x" type="submit"><?= i('Publish'); ?></button><?php if ($c['deep'] > 0): ?> <a class="button comment-reply:x" href="<?= $url->clean . $url->query('&amp;', ['parent' => false]) . '#' . $c['anchor'][1]; ?>"><?= i('Cancel'); ?></a><?php endif; ?><?php if ($advance && !empty($c['user'])): ?> <span class="button comment-user">
