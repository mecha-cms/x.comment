...
function _fn_get_comment_property($v, $n = null) {
    $n = $n ?: Path::N($v);
...
function _fn_get_comment($path, $key = null, $fail = false, $for = null) {
    if (!file_exists($path)) {
        return $fail;
    }
    ...
    if (is_dir($data)) {
        if ($for === null) {
            foreach (g($data, 'data') as $v) {
                $n = Path::N($v);
                $output[$n] = e(_fn_get_comment_property($v, $n));
            }
        } else if ($v = File::exist($data . DS . $for . '.data')) {
            $output[$for] = e(_fn_get_comment_property($v, $for));
        }
    }
    return !isset($key) ? $output : (array_key_exists($key, $output) ? $output[$key] : $fail);
}

function fn_get_comments($folder = COMMENT, $state = 'page', $sort = [-1, 'time'], $key = null) {
    $output = [];
    $by = is_array($sort) && isset($sort[1]) ? $sort[1] : null;
    if ($input = g($folder, $state)) {
        foreach ($input as $v) {
            $output[] = _fn_get_comment($v, null, false, $by);
        }
...
Get::_('comments', 'fn_get_comments');